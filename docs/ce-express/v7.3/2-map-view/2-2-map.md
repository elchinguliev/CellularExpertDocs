# 2.2 Map
## 2.2.1 Search bar

![Image p254](../../../assets/images/ce-express/user-guide-v73/p254-img1.png)

Allows searching for an address, place, or location.

## 2.2.2 Zoom in

![icon](../../../assets/images/ce-express/user-guide-v73/p255-img1.png) Zooms in on the map.

## 2.2.3 Zoom out

![icon](../../../assets/images/ce-express/user-guide-v73/p255-img2.png) Zooms out on the map.

## 2.2.4 Fine zoom

![icon](../../../assets/images/ce-express/user-guide-v73/p255-img3.png) When enabled, zoom steps are not locked to standardized scale levels. This allows to zoom more finely
when more precision is needed.

## 2.2.5 Compass

![icon](../../../assets/images/ce-express/user-guide-v73/p255-img4.png) Shows the current map orientation and allows resetting north-up.

## 2.2.6 3D/2D Button

![icon](../../../assets/images/ce-express/user-guide-v73/p255-img5.png) Switches between 2D and 3D map view.

## 2.2.7 Legend

![icon](../../../assets/images/ce-express/user-guide-v73/p256-img1.png) Opens the legend panel showing all visible layers and their symbols.

## 2.2.8 Home

![icon](../../../assets/images/ce-express/user-guide-v73/p256-img2.png) Returns the map to the workspace extent.

## 2.2.9 Mini calculation task window

Mini calculation task window displays the result of the most recent calculation. Unlike the full Prediction
History tool, this window does not list all past calculations, it only shows the latest created task.

![Image p256](../../../assets/images/ce-express/user-guide-v73/p256-img4.png)

## 2.2.10 Active side switch

Shows up when two tools, which have map clicking enabled, are opened at the same time. You may then
switch between left and right sides, to select which tool is active. For example: Features tool on the left,
and Profile tool on the right. When the active side switch is flipped to the left, clicking on the map will start
feature selection, rather than draw a profile.

![Image p256](../../../assets/images/ce-express/user-guide-v73/p256-img3.png)

## 2.2.11 Quick menu

The quick menu opens when middle-clicking on the map. If the click is on a feature, it provides actions such
as copying coordinates, adding, editing, moving, duplicating, or deleting the feature. If the click is not on a feature, it shows only copying coordinates and adding new items.

![Image p257](../../../assets/images/ce-express/user-guide-v73/p257-img1.png)

