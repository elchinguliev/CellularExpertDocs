# Software Installation and Upgrades

## Installation Order

1. **ArcGIS Pro**
2. **Cellular Expert**

## Installing Cellular Expert

Installation requires administration rights.

1. Right-click **ceProSetup.msi** and select **Open**.

   ![ceProSetup.msi context menu](../../assets/images/ce-pro/training-00/p003-img1.png)

2. Follow the **Cellular Expert 4.0.8 for ArcGIS Pro** installer.

   ![Cellular Expert installer wizard](../../assets/images/ce-pro/training-00/p003-img2.png)

3. The installer displays a Quick Start guide:

   ![Cellular Expert 4.0.8 for ArcGIS Pro Information dialog — Quick Start guide](../../assets/images/ce-pro/training-00/p003-img3.png)

4. In **Task Manager → Services**, locate `CE_Pro_Prediction_Service`, right-click it, and select **Start**.

   ![Starting CE_Pro_Prediction_Service from the Services list](../../assets/images/ce-pro/training-00/p003-img4.png)

## Activating the Licence

1. Open an empty ArcGIS Pro project.

   ![ArcGIS Pro account sign-in panel](../../assets/images/ce-pro/training-00/p004-img1.png)

2. Open **Settings → Licensing**.
3. Enable the **Cellular Expert** extension.

   ![External Extensions — Cellular Expert enabled](../../assets/images/ce-pro/training-00/p004-img2.png)

4. Send your **User Key**, then click **Activate License**.

   ![Cellular Expert License dialog](../../assets/images/ce-pro/training-00/p004-img3.png)

## License Information

1. Open **ArcGIS Pro**.
2. Add a new map to enable the CE tabs.
3. Open **Licence Information**.

   ![CE RCP ribbon — Licence Information button](../../assets/images/ce-pro/training-00/p005-img2.png)

   ![Cellular Expert License dialog](../../assets/images/ce-pro/training-00/p005-img1.png)

## Upgrading Cellular Expert

1. **Delete Cellular Expert**
2. **Install Cellular Expert**

## Uninstallation

1. **Cellular Expert**
2. **ArcGIS Pro**

## Upgrading ArcGIS Pro

1. Run the ArcGIS Pro upgrade (Cellular Expert uninstallation is not required).
