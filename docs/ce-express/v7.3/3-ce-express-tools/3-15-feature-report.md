...
| Option | Description |
|---|---|
| Map page | Whether to add a page with a view of the map to the PDF report output. |
...
| Option | Description |
|---|---|
| Enable | Whether to draw the arrows originating from selected cells. |
| Arrow length | Length of arrows in meters. |

**Perimeter circles**

| Option | Description |
|---|---|
| Enable | Whether to draw perimeter circles around selected features. |
| Feature type | Feature type which is used as the origin for perimeter circles. |
| Draw radius line | Whether to draw a line within the circles marking the radius of the circle. A label of the radius length is displayed above the dashed line. |

![Image p139](../../../assets/images/ce-express/user-guide-v73/p139-img1.png)

**Perimeter radius lengths**

Multiple perimeters around each feature may be drawn. This list allows you to define the length of each perimeter radius.

**Omen points**

| Option | Description |
|---|---|
| Place omen points | This button places omen points on the intersection points of cell arrow lines and radius circles. |
| Name prefix | Each placed point is numbered with 1 – n in the name (1, 2, 3, …, n). This prefix is added to each name. For example, if the prefix is defined as "omen", the created omen points will be named as "omen 1", "omen 2", "omen n". |
